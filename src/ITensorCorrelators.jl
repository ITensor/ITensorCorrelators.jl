...
function correlator_repeat(
  ...
  for i in is #choose first site ("A")
    ...
    op_psi_i = apply(op(ops[1], s[i]), psi[i]) #apply first operator "A" in position i
    L_i = (i>1 ? delta(dag(ln[i-1])',ln[i-1]) : 1.)  #left system to conserve between contractions
...
export correlator
export correlator_repeat

end

#TODO
#1) make it work for all hilbert spaces
#2) Add possibility of two operators on same site
